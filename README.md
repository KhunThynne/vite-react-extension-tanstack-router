...
- **Local-First Database (RxDB + TanStack DB)**: Real-time, reactive local database using IndexedDB (via Dexie). State magically syncs across different extension contexts (e.g., Background Scripts and Popups) out of the box using `multiInstance: true`. Includes built-in support for schema migrations, Z-Schema validation, and live replication.
